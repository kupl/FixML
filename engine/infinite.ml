...
module Static = struct
	...
	let rec matching_args : exp -> (id * arg list) -> bool
	= fun exp (f,args) ->
  ...
  let rec preprocess :  exp -> (exp * arg list)
  = fun e ->
    match e with
    | EFun (arg,e) -> 
	...
	let rec check_exp2 : ((id * arg list)) -> exp -> bool
	= fun func exp ->
		match exp with
    ...
    | EApp (e1, e2) -> 
      matching_args exp func || check_exp2 func e1 || check_exp2 func e2 (* up to here *)
