  ...
  = fun (exp, var) -> match exp with
	...
	|Times lst ->(match lst with|[]->Const 0 |hd::tl ->if diff(hd,var)=Const 0 then Times[hd;diff(Times tl,var)] else Sum[Times([diff(hd,var)]@tl);Times[hd;diff(Times tl,var)]]) ;; 
